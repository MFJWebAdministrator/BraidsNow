...
export function WorkHoursSection({ workHours, onUpdate }: WorkHoursSectionProps) {
  return (
    <div className="space-y-6">
      {DAYS.map(({ key, label }) => (
        <div key={key} className="flex flex-col md:flex-row md:items-center justify-between p-4 rounded-lg bg-gray-50 gap-4 md:gap-0">
          <div className="flex items-center space-x-4">
            <Switch
              checked={workHours[key].isEnabled}
              onCheckedChange={(checked) => 
                onUpdate(key, { ...workHours[key], isEnabled: checked })
              }
            />
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-2 sm:space-y-0 sm:space-x-4 w-full sm:w-auto">
            <Select
              value={formatTime(workHours[key].start.hour, workHours[key].start.minute)}
              onValueChange={(value) => {
            ...
            <Select
              value={formatTime(workHours[key].end.hour, workHours[key].end.minute)}
              onValueChange={(value) => {
                const { hour, minute } = parseTime(value);
                onUpdate(key, {
                  ...workHours[key],
