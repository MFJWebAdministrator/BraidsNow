...
import { Search, MessageSquare, Send, Loader2, Paperclip } from "lucide-react";
...
export function MessagesContent() {
    ...
    const filteredThreads = threads.filter((thread) => {
        ...
        return participantDetails.name
            .toLowerCase()
    ...
    const handleSendMessage = async () => {
        ...
        try {
            ...
            const recipientId = currentThread.participants.find(
                (p) => p !== user.uid
            );
            ...
            if (selectedImages.length > 0) {
                imageUrls = await Promise.all(
                    selectedImages.map(async (file) => {
                        const imageRef = ref(
                            storage,
                            `profile-images/${user?.uid}/${messageRef.id}-${Date.now()}`
                        );
    ...
    const handleCropComplete = (croppedFile: File) => {
        ...
        setImagePreviews((prev) =>
            [...prev, URL.createObjectURL(croppedFile)].slice(0, 3)
        );
        // Move to next image in queue
        const [, ...rest] = croppingQueue;
        if (rest.length > 0) {
    ...
    const handleImageChange = async (
        ...
        for (
            let i = 0;
            i < files.length && selectedImages.length + newFiles.length < 3;
            i++
        ) {
            let file = files[i];
                        ...
                        {filteredThreads.length > 0 ? (
                            filteredThreads.map((thread) => {
                                ...
                                const participantDetails =
                                    thread.participantDetails[otherParticipant];
                                    ...
                                    >
                                        <div className="flex items-center gap-3">
                                            <Avatar>
                                                <AvatarImage
                                                    src={
                                                        participantDetails.image
                                                    }
                                                />
                                                <AvatarFallback>
                                                    {participantDetails.name[0]}
                                                </AvatarFallback>
                                            </Avatar>
                            ...
                            <div className="text-center py-12">
                                <div className="bg-gray-50 rounded-2xl p-8">
                                    ...
                                    <p className="text-gray-600 tracking-normal text-sm">
                                        Messages from your stylists will appear
                                        here
                                    </p>
                                </div>
                            </div>
                                ...
                                <div className="flex-1 overflow-y-auto space-y-4 mb-4">
                                    {messages.map((message) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                <div
                                                    className={`flex items-start gap-2 max-w-[70%] ${isSender ? "flex-row-reverse" : ""}`}
                                                >
                                                    ...
                                                    <div>
                                                        ...
                                                        <div
                                                            className={`text-xs mt-1 ${isSender ? "text-right" : ""}`}
                                                        >
                                                            {format(
                                                                new Date(
                                                                    message.createdAt
