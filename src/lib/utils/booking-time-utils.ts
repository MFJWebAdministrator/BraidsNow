...
export const calculateTotalBookingTime = (
  ...
  const totalMinutes = serviceDuration.hours * 60 + serviceDuration.minutes + bufferTime;
  return {
...
export const checkTimeSlotOverlap = (
  ...
  const slot1BufferEnd = calculateEndTime(slot1End, { hours: 0, minutes: bufferTime });
  const slot2BufferEnd = calculateEndTime(slot2End, { hours: 0, minutes: bufferTime });
