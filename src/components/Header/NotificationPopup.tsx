...
export function NotificationPopup() {
  const { user } = useAuth();
  const { notifications, loading, markAllAsRead, markAsRead, markAsSeen, unreadCount } = useNotifications();
    ...
    <Popover onOpenChange={handlePopoverOpen}>
      ...
      <PopoverContent className="w-80 p-0 bg-white" align="end" sideOffset={5}>
        <div className="border-b border-gray-200 bg-[#3F0052] rounded-t-lg flex items-center justify-between">
          <h3 className="p-3 text-sm font-medium text-white">Notifications</h3>
          {unreadCount > 0 && (
            <Button
              variant="ghost"
              size="sm"
