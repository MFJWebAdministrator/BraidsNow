...
import { addToFavorites, removeFromFavorites, getFavoriteStylists } from '@/lib/firebase/client/favorites';
...
export function useFavorites() {
  ...
  const toggleFavorite = async (stylist: Stylist) => {
    ...
    try {
      ...
      if (isFavorite) {
        ...
        setFavorites(prev => prev.filter(fav => fav.id !== stylist.id));
        toast({
  ...
  return {
    ...
    loading,
    toggleFavorite,
    isFavorite: (stylistId: string) => favorites.some(fav => fav.id === stylistId)
  };
}