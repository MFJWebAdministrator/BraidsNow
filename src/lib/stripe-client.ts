...
export async function createConnectAccount(userId: string) {
    try {
        ...
        // Use Firebase Functions HTTP callable
        const createConnectAccountFn = httpsCallable(
            functions,
...
export async function checkSubscriptionStatus(userId: string) {
    try {
        ...
        if (stylistDoc.exists()) {
            ...
            if (subscription) {
                return {
                    ...
                    stripeAccountStatus:
                        data.stripeAccountStatus || "not_created",
                };
            }
        ...
        const checkAccountStatusFn = httpsCallable(
            functions,
            "checkAccountStatus"
        );
        ...
        console.log(
            "Subscription status response from function:",
            response.data
        );
...
// Remove the commented out loadStripeX function and Window interface
