...
export function useStyleBoard(userId: string) {
  ...
  const getCategoryKey = (category: StyleImage['category']): keyof StyleBoard => {
    switch (category) {
      case 'current':
  ...
  const uploadImage = async (file: File, category: StyleImage['category']) => {
  ...
  const deleteImage = async (imageId: string, category: StyleImage['category']) => {
    if (!userId || !styleBoard) {
      throw new Error('Style board not initialized');
    }
