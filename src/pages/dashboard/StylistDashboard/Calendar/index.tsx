...
export function CalendarPage() {
  ...
  const generateBreakEvents = () => {
    ...
    console.log(">>>>>>>>>>>>>>>>> schedule Break >>>>>>>>>>>>>>>>>>>", schedule.breaks)
  ...
  const generateBufferZoneEvents = () => {
    ...
    stylistAppointments
      ...
      .forEach((appointment: Appointment) => {
        ...
        const serviceDuration = (appointment as any).service?.duration || { hours: 1, minutes: 0 };
        const serviceEndDate = new Date(startDate.getTime() + (serviceDuration.hours * 60 + serviceDuration.minutes) * 60000);

        // Generate buffer before appointment
        if (schedule.bufferTime.before > 0) {
          const bufferStartDate = new Date(startDate.getTime() - schedule.bufferTime.before * 60000);
          bufferEvents.push({
        ...
        if (schedule.bufferTime.after > 0) {
          const bufferEndDate = new Date(serviceEndDate.getTime() + schedule.bufferTime.after * 60000);
          bufferEvents.push({
  ...
  const generateUnavailableTimeSlots = () => {
    ...
    dayNames.forEach((day, dayIndex) => {
      const workHours = schedule.workHours[day as keyof typeof schedule.workHours];
  ...
  const calendarEvents = useMemo(() => {
    const appointmentEvents = stylistAppointments
      ? stylistAppointments
          ...
          .map((appointment: Appointment) => {
            ...
            const duration = (appointment as any).service?.duration || { hours: 1, minutes: 0 };
    ...
    return [...unavailableEvents, ...bufferEvents, ...breakEvents, ...appointmentEvents];
  }, [stylistAppointments, schedule.breaks, schedule.bufferTime, schedule.workHours]);
        ...
        <div className="flex-1 overflow-auto p-3 sm:p-4 md:p-6">
          <div className="max-w-7xl mx-auto space-y-4 sm:space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                <h1 className="text-2xl sm:text-3xl font-light text-[#3F0052]">Calendar</h1>
            ...
            <Card className="p-3 sm:p-4">
              <div className="flex flex-wrap items-center gap-3 sm:gap-6 text-xs sm:text-sm">
                ...
                <div className="flex items-center space-x-2">
                  <div className="w-3 h-3 sm:w-4 sm:h-4 rounded" style={{ backgroundColor: 'rgba(63, 0, 82, 0.3)' }}></div>
                  <span>Buffer Zones</span>
                </div>
                <div className="flex items-center space-x-2">
                  <div className="w-3 h-3 sm:w-4 sm:h-4 rounded" style={{ backgroundColor: 'rgba(156, 163, 175, 0.3)' }}></div>
                  ...
                  <div className="min-w-[600px] sm:min-w-0">
                    <FullCalendar
                      ...
                      datesSet={(dateInfo) => {
                        ...
                        if (isMobile && dateInfo.view.type === 'timeGridWeek') {
                          dateInfo.view.calendar.changeView('dayGridMonth');
                        }
                      }}
                      ...
                      eventDidMount={(info) => {
                        ...
                        } else if (extendedProps?.isUnavailable) {
