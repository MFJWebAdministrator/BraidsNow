    ...
    try {
        ...
        if (stylistSchedule?.bufferTime) {
            ...
            existingBookings.forEach((booking) => {
                ...
                const bookingEndTime = calculateEndTime(
                    booking.time,
                    (booking as any).service?.duration || {
                        hours: 0,
                        minutes: 0,
                    }
                );
                ...
                const bookingBufferEndTime = calculateEndTime(bookingEndTime, {
                    hours: 0,
                    minutes: bufferAfter,
                });

                // Add buffer time to the end of the new booking
                const newBookingBufferEndTime = calculateEndTime(newEndTime, {
                    hours: 0,
                    minutes: bufferAfter,
                });
                ...
                const hasBufferConflict =
                    (newStartTime < bookingBufferEndTime &&
                        newBookingBufferEndTime > booking.time) ||
                    (booking.time < newBookingBufferEndTime &&
                        bookingBufferEndTime > newStartTime);

                if (hasBufferConflict) {
                    conflicts.push(
                        `Conflicts with appointment (including buffer time): ${booking.clientName}`
                    );
                }
            });
        } else {
            ...
            existingBookings.forEach((booking) => {
                ...
                const hasOverlap =
                    (newStartTime < bookingEndTime &&
                        newEndTime > booking.time) ||
                    (booking.time < newEndTime &&
                        bookingEndTime > newStartTime);

                if (hasOverlap) {
                    conflicts.push(
                        `Conflicts with existing appointment: ${booking.clientName}`
                    );
                }
            });
        }
        ...
        if (stylistSchedule?.breaks) {
            ...
            stylistSchedule.breaks.forEach((breakItem) => {
                if (breakItem.days.includes(dayOfWeek as any)) {
                    ...
                    if (hasBreakConflict) {
                        conflicts.push(
                            `Conflicts with scheduled break: ${breakItem.name}`
                        );
                    }
                }
            });
        ...
        if (stylistSchedule?.workHours) {
            ...
            const dayOfWeek = dayNames[
                selectedDate.getDay()
            ] as keyof typeof stylistSchedule.workHours;
...
export const checkScheduleConflicts = async (
    ...
    try {
        ...
        const conflicts = existingBookings.filter((booking) => {
            ...
            // TODO: what about the other failed statuses 
...
export const getAvailableTimeSlots = (
    ...
    const currentTime = new Date(2000, 0, 1, startHour, startMinute);
